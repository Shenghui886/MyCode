/*
	...
	the [errors package](https://pkg.go.dev/errors)
	and this [blog post](https://go.dev/blog/go1.13-errors)
...
func f(arg int) (int, error) {
	if arg == 42 {
		// errors.New construsts abasic error value
		// with the given error message.
		return -1, errors.New("can't work with 42")
	}
	// A nil value in the error position nidicates that
	// there was no error.
	return arg + 3, nil
}
...
Wrapped errors create a logical chain
(A wraps B, which wraps C, etc.) that
can be queried with functions like errors.Is and errors.As.
*/
...
func main() {
	...
	/*
		errors.Is checks that a given
		error (or any error in its chain)
		matches a specific error value.

		This is especially useful with wrapped
		or nested errors, allowing you to identify
		specific error types or sentinel errors
