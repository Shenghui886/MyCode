/*
Go otthers built-in support for JSON encoding and
...
func main() {
	/*
		...
		string. Here are some examples for atomic values.
	*/
	...
	*/
	...
	/*
		...
		to convert them to their appropriate type. For example
		here we convert the value in 'num' to the expected 'float64'
		type.
	*/
	num := dat["num"].(float64)
	fmt.Println(num)
	...
	*/
	...
	/*
		In the examples obove we always used bytes and strings as
		intermediates between the data and JSON representation
		on standard out. We can also stream JSON encodings
		directly to 'os.Writer's like 'os.Stdout' or even HTTP
		response bodies.
	*/
	...
	/*
		Streaming reads from 'os.Reader's like 'os.Stdin' or HTTP
		request bodies is donw with 'json.Decoder'.
	*/
