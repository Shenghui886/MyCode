	...
	*/
	go func() {
		for {
			j, more := <-jobs
			if more {
				fmt.Println("receive job", j, "\tmore is:", more)
			} else {
	...
	/*
		This sendds 3 jobs to the worker over the 'jobs' channel,
		then closes it.
	*/
	for j := 1; j <= 3; j++ {
		jobs <- j
	...
	/*
		...
		(https://gobyexample.com/channel-synchronization) approach
		we saw earlier.
	*/
	<-done
	...
	/*
		...
		returning the zero value of the underlying type. The
		optional second return value is 'true' if the value received
		was delivered by a successful send operation to the channel,
		or 'false' if it was a zero value generated because the channel
		is closed and empty.
	*/
	_, ok := <-jobs
