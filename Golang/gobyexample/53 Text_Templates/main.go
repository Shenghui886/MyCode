...
func main() {
	/*
		We can create a new template and parse its body from a
		string. Templates are a mix of static text and "actions"
		enclosed in {{...}} that are used to dynamically insert
		content.
	*/
	t1 := template.New("t1")
	...
	*/
	...
	/*
		The same applies to maps; with maps there is no
		restriction on the case of key names.
	*/
	...
	/*
		...
		A value is considered false if it's the default value of
		a type, such as 0, an empty string, nil pointer, etc. This
		sample demonstrates another feature of templates: using '-'
		in action to trim whitespace.
	*/
	t3 := Create("t3",
		"{{if . -}} yes {{else -}} no {{end}}\n")
	...
	/*
		...
		channels. Inside the range block {{.}} is set to the current
